...
def format_examples(example):
    try:
        choices = "\n".join([f"({chr(65+i)}) {text}" for i, text in enumerate(example['choices']['text'])])
        example["text"] = f"### Question: {example['question_stem']}\n{choices}\n### Answer: {example['answerKey']}"
    except Exception as e:
...
if __name__ == "__main__":
    ...
    prepared_dataset.save_to_disk("data/sft_openbookqa")
    print("Dataset saved to disk.")
    loaded_dataset = load_from_disk("data/sft_openbookqa")
