...
def load_model(model_location):
    ...
    elif "compressed_llama" in model_location:
        print("INSIDE ELIF")
        model = LlamaCausalLMTensor.from_pretrained(model_location)
    else:
        # TODO: Remove this hardcoding and get correct loader from model_type in config.json
        print("INSIDE ELSE")
        model = Mixtral8x7Tensor.from_pretrained(model_location)
    print(model)
    return model
...
def evaluate_model(model_name, model_args, datasets, num_fewshot, batch_size, device, output_dir, limit=1.0):
    ...
    # Initialize Accelerator with DeepSpeed configuration
    accelerator = Accelerator(deepspeed_plugin=True)
    accelerator.deepspeed_config = {
        "deepspeed": {
            ...
            "train_batch_size": batch_size if isinstance(batch_size, int) else 1,
            "fp16": {
                "enabled": True
            },
            "bf16": {
...
model_name = "/home/ubuntu/llm_training/outputs/sft_openbookqa"
model_args = {"revision": "main", "dtype": "float", "parallelize": False}
datasets = ["mmlu", "hellaswag", "boolq"]  # Add your datasets here
num_fewshot = 0
batch_size = 4  # Set batch size to a fixed integer for DeepSpeed
device = "cuda:7"
